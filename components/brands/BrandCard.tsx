  ...
  return (
    <Link href={`/brands/${brand.slug}`} className="group block h-full">
      <div className="h-full bg-white rounded-2xl border border-slate-200 hover:border-[#005AA9]/50 hover:shadow-xl transition-all duration-300 overflow-hidden flex flex-col">
        ...
        {/* Logo Section */}
        <div className="relative h-28 sm:h-40 bg-slate-50 flex items-center justify-center p-3 sm:p-6 shrink-0">
          {brand.featured && (
            <span className="hidden md:flex absolute top-3 right-3 items-center gap-1 px-2 py-0.5 rounded-full bg-amber-50 border border-amber-200 text-amber-600 text-[10px] font-bold">
        ...
        <div className="p-3 sm:p-4 flex flex-col flex-1">
          <h3 className="text-xs sm:text-sm font-bold text-[#002244] mb-1.5 sm:mb-2 line-clamp-1 group-hover:text-[#005AA9] transition-colors">
            {brand.name}
          </h3>
          ...
          {/* Short Description */}
          <p className="hidden md:block text-xs text-slate-500 line-clamp-2 mb-4 flex-1">
            {brand.description}
          </p>

          {/* CTA */}
          <div className="flex items-center text-[#005AA9] font-bold text-xs group-hover:text-[#00aaff] transition-colors mt-auto pt-1 sm:pt-0">
            Learn More
            <ArrowRight
              size={13}
